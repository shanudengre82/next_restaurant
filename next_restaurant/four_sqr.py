...
def get_four_sqr_api(client_id,client_secret, categoryId, runs, radius):
    ...
    for i in range(runs):
        ...
        for n in range(len(response['response']['groups'][0]['items'])):
            ...
            four_sqr_name = response['response']['groups'][0]['items'][n]['venue']['name']
            four_sqr_category = (response['response']['groups'][0]['items'][n]['venue']['categories'][0]['name'])
            four_sqr_address = ' '.join(response['response']['groups'][0]['items'][n]['venue']['location']['formattedAddress'])
            four_sqr_lat = response['response']['groups'][0]['items'][n]['venue']['location']['lat']
            four_sqr_lng = response['response']['groups'][0]['items'][n]['venue']['location']['lng']
            df_temp.loc[len(df_temp)] = [four_sqr_id, four_sqr_name, four_sqr_address, four_sqr_lat, four_sqr_lng, four_sqr_category]
