...
def percent_of_good_restaurants(df, rating, popularity):
    ...
    good_restaurants = df[df['rating'] >= rating]
    number_of_good_restaurants = len(good_restaurants[ good_restaurants['user_ratings_total'] >= popularity])
    percentage_of_good_restaurants = number_of_good_restaurants / total_num_of_restaurants
...
def stats_per_cuisine(df, cuisine, rating, popularity):
    ...
    if cuisine == 'All':
        ...
        for cuisine in cuisines:
            ...
            lst_.append(round(cuisine_type['names_clean'].count() /df['names_clean'].count(),2))
            lst_.append(round(good_cuisine_type_df['names_clean'].count() /cuisine_type['names_clean'].count(), 2))

            # Make it a df

            splitted = [lst_[i:i + 4] for i in range(0, len(lst_), 4)]
            cuisines_df = pd.DataFrame(splitted)
            cuisines_df.columns = ['cuisine', 'number_restaurants_in_Berlin', '%_all_restaurants_in_Berlin',
            '%_considered_good']
            cuisines_df = cuisines_df.sort_values(by = ['%_all_restaurants_in_Berlin'], ascending = False)
...
def stats_per_hood(df, rating, popularity):
    ...
    for hood in hoods:
        ...
        lst_2.append(round(hood_df['names_clean'].count()/df['names_clean'].count(),2))
        lst_2.append(round(good_in_hood_df['names_clean'].count()))
        lst_2.append(good_in_hood_df['names_clean'].count()/good_restaurants_df['names_clean'].count())

        # Make it a df
        splitted_ = [lst_2[i:i + 5] for i in range(0, len(lst_2), 5)]
        hoods_df_ = pd.DataFrame(splitted_)
        hoods_df_.columns = ['district','number_of_restaurants','%_all_berlin_restaurants', 'number_good_restaurants', '%_all_good_restaurants']
...
def stats_per_hood_and_cuisine(df, rating, popularity):
    ...
    for hood in hoods:
        for cuisine in cuisines:
            ...
            all_good_cuisines_df = all_good_cuisines[all_good_cuisines.user_ratings_total >= popularity]

            lst_3.append(hood)
            lst_3.append(cuisine)
            lst_3.append(cuisine_df['food_type'].count())
            lst_3.append(round(cuisine_df['food_type'].count()/hood_df['food_type'].count(),2))
            lst_3.append(round(good_df['names_clean'].count()/cuisine_df['food_type'].count(), 2))
            lst_3.append(round(good_df['names_clean'].count()/all_good_cuisines_df['names_clean'].count(), 2))
            ...
            hoods_cuisine.columns = ['district', 'cuisine', 'count',
                                        '%_restaurants_in_district', '%_considered_good',
                                        '%_all_good_restaurants_for_this_cuisine_in_berlin']
            hoods_cuisine = hoods_cuisine.sort_values(by= ['district', 'count'], ascending = False)
...
def stats_per_cuisine_and_hood(df, rating, popularity):
    ...
    for cuisine in cuisines:
        for hood in hoods:
            ...
            lst_4.append(round(
                    hood_df['names_clean'].count() / all_cuisines['names_clean'].count(), 2))
            lst_4.append(
                round(good_df['names_clean'].count() / hood_df['names_clean'].count(), 2))
            lst_4.append(
                round(
                    good_df['names_clean'].count() /
                    all_good_cuisines_df['names_clean'].count(), 2))

    # make the list a dataframe
    list_splitted = [lst_4[i:i + 6] for i in range(0, len(lst_4), 6)]
    cuisine_hoods = pd.DataFrame(list_splitted)
    cuisine_hoods.columns = ['cuisine', 'district', 'count', 'percent_all_restaurants_of_berlin',
                             '%_considered_good','%_all_good_restaurants_for_this_cuisine_in_berlin']
