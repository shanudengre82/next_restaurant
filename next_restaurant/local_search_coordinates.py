...
def generating_circular_coordinates(df, lat=52.5607405, lng=13.3808273, radius=100):
    ...
    box_coordinates.append([lat + (lat - lat_max), lng + lng-lng_max])  # opposite to first
    ...
    lat_distance = distance(lat1 = box_coordinates[0][0],
                            lat2 = box_coordinates[2][0],
                            lng1 = box_coordinates[0][1],
                            lng2 = box_coordinates[2][1])

    lng_distance = distance(lat1 = box_coordinates[0][0],
    ...
    for i in np.linspace(box_coordinates[0][0], box_coordinates[2][0], lat_divisions):
        for j in np.linspace(box_coordinates[0][1], box_coordinates[3][1], lng_divisions):
            box_coordinates_inside.append([i, j])

    box_sorted = []

    distance_max = distance(lat1=lat_max, lat2 = lat, lng1 = lng_max, lng2 = lng)
...
def locating_best_place_based_on_distance(df, box: list):
    ...
    for i in box:
        ...
        for j in range(len(df)):
            distance_ = distance(lat1=df.iloc[j]["lat"], lat2 = i[0], lng1 = df.iloc[j]["lng"], lng2 = i[1])
            count.append(distance_)
